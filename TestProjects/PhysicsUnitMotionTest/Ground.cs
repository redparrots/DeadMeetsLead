        ...
        public void ConstructPieces()
        {
            ...
            for (int y = 0; y < NPieces.Height; y++)
                for (int x = 0; x < NPieces.Width; x++)
                {
                    ...
                    mo.LocalBounding = new Common.Bounding.GroundPiece
                    {
                        Bounding = new Common.Bounding.Chain
                        {
                            Shallow = true,
                ...
                XMesh = new MeshConcretize
                {
                    MeshDescription = new Graphics.Software.Meshes.MeshFromHeightmap
                    {
                        Grid = new Graphics.Software.Meshes.Grid
                        {
                            ...
                            Size = Size,
                            NWidth = Math.Max(1, (int)((Ground.Heightmap.GetLength(1) - 1) * HeightmapSelection.Width)),
                            NHeight = Math.Max(1, (int)((Ground.Heightmap.GetLength(0) - 1) * HeightmapSelection.Height)),
